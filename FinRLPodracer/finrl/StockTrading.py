...
class StockEnvNAS89(gym.Env, ABC):
    ...
    def __init__(self, cwd='./envs/finrl', gamma=0.999, turbulence_thresh=30, min_stock_rate=0.1,
                 ...
                 start_date='2009-01-01', end_date='2009-12-02', data_gap=4, reward_scaling=2 ** -13,
                 ticker_list=None, tech_indicator_list=None, initial_stocks=None, if_eval=False, scrap=False, timeframe="minute"):
        ...
        data_arrays = self.load_data(cwd, ticker_list, tech_indicator_list, start_date, end_date, scrap, timeframe)
    ...
    def load_data(self, cwd='./envs/finrl', ticker_list=None, tech_indicator_list=None,
        ...
        if os.path.exists(data_path_array) and False:
            ...
            if os.path.exists(data_turb_array):
                turbulence_ary = load_dict['turbulence_ary'].astype(np.float32)
            else:
                print("no path found for turbulence_ary, stop.")
                sys.exit(1)
        else:
            raw_data2_df = self.get_raw_data2(raw_data1_path, raw_data2_path, start_date, end_date,
                                              ticker_list, tech_indicator_list, scrap, timeframe)


            print(raw_data2_df.head())
            ...
            # print(tech_ary.shape)  # (528026, 93 * 7)  # chque "example" c'est un datapoint, dans la matrice
            #                                               des STOCKS * indicators


            # print(raw_data2_df.date)  # to show the start_date and end_date
            # print(raw_data2_df.columns)  # to show the items name
            price_ary, tech_ary, turb_ary = self.convert_df_to_ary(raw_data2_df, tech_indicator_list)
            price_ary, tech_ary, turb_ary = self.deal_with_split_or_merge_shares(price_ary, tech_ary, turb_ary)
    ...
    def get_raw_data2(self, raw_data1_path, raw_data2_path, start_date, end_date,
                      ticker_list, tech_indicator_list, scrap, timeframe):
        if os.path.exists(raw_data2_path) and False:
            ...
            raw_df = self.get_raw_data(raw_data1_path, ticker_list, start_date, end_date, scrap=scrap, timeframe=timeframe)
    ...
    def deal_with_split_or_merge_shares(self, price_ary, tech_ary, turb_ary):

        print(price_ary.shape)  # (3121, 11) (at each day (3121 in total) we have 11 close prices where 11 is the number of diff shares/stocks)
        print(tech_ary.shape)   # (3121, 77) (77 coz 11x7 where 7 is the number of indicators)
        print(turb_ary.shape)   # (3121, 11)
 
        tech_ary = tech_ary.reshape((tech_ary.shape[0], -1, len(self.tech_indicator_list)))

        data_idx = list(range(price_ary.shape[1])) # [0 1 .... 10]
        ...
        for j in range(price_ary.shape[1]):
            ...
            x = price_ary[:, j] # retrieve the share (j) prices
        ...
        for j in range(turb_ary.shape[1]):
            x = turb_ary[:, j] # 
            x = self.fill_nan_with_next_value(x)  # fill_nan_with_next_value   SE
            turb_ary[:, j] = x
    ...
    @staticmethod
    def get_raw_data(raw_data_path, ticker_list, start_date, end_date, scrap, timeframe):
        print("hhhaaaaa")
        ...
        if os.path.exists(raw_data_path):
            ...
            print("| YahooDownloader: start downloading data ("+timeframe+")")
            ...
            # df = yfp.scrap_data(ticker_list, start_date, end_date)
            # print("| YahooDownloader: start scrapping data ("+timeframe+")")
            # print(df)
            
            # str_tickers = "_".join(tuple(ticker_list))
    ...
    def convert_df_to_ary(df, tech_indicator_list):
        ...
        for day in df["day"].unique():
            ...
            turb_items = [item[tech].values.tolist() for tech in ["turbulence"]]
            turb_items_flatten = sum(turb_items, [])
            turb_ary.append(turb_items_flatten)


        tech_ary = np.array(tech_ary)
        price_ary = np.array(price_ary)
        turb_ary = np.array(turb_ary)
        print(f'| ary.shape price, tech, turbulence: {price_ary.shape} {tech_ary.shape} {turb_ary.shape}')
        ...
        for indicator in self.tech_indicator_list:
            ...
            for i in range(len(unique_ticker)):
                try:
                    ...
                    print(e)
            # print("indicator_dfindicator_dfindicator_df")
            # print(indicator_df)
            # exit()
