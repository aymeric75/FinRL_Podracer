...
def detect_date_format(date_str):
    """Detect date format and return the correct datetime object."""
    possible_formats = ["%Y-%m-%d", "%b %d, %Y", "%B %d, %Y"]  # Add other formats if needed
    ...
    # If none of the formats match, try using dateutil parser as a fallback
    try:
...
class YahooFinanceProcessor:
    ...
    def fetch_stock_data(self, stock_name, period1, period2) -> pd.DataFrame:
        ...
        df["date"] = df["date"].apply(detect_date_format)
