...
def demo_custom_env_finance_rl_nas89():  # 1.7+ 2.0+
    ...
    args.gamma = 0.999
    args.env = StockEnvNAS89(if_eval=False, gamma=args.gamma, turbulence_thresh=30, scrap=True, timeframe="day", tech_indicator_list = tech_indicator_list)
    args.eval_env = StockEnvNAS89(if_eval=True, gamma=args.gamma, turbulence_thresh=15, scrap=True, timeframe="day", tech_indicator_list = tech_indicator_list)
    ...
    if_single_env = 1
    if if_single_env:
        args.gpu_id = "0" #int(sys.argv[-1][-4])
